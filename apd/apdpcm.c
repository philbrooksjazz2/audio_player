...
//    pCard->dfx_handle[1] = NULL;  
    ...
    if (tmphandle != -1) 
    {
        ...
        nSndParam = 2;   /* PCM handler only supports stereo  */
        ioctl(tmphandle, SNDCTL_DSP_CHANNELS, &nSndParam); 
...
int pcmSubmitBuffer(ACard *pCard, unsigned char *frame_data, int nSize) 
{
...
//    pCard->pcmBufferSize = 512;

    nBytesPerBuf = pCard->pcmBufferSize * PCM_FRAMESIZE;
    ...
    while (TRUE) 
    {
        ...
        if (pCard->buffer_pending[0] < (PCM_NUMPCMBUF * nBytesPerBuf)) 
        {
            break;
        }
    ...
    {
        bufpos = (( nBytesPerBuf ) * (pCard->xferbuf_pos[0])) + 
             pCard->buffer_pending[0];

        if (bufpos >= (PCM_NUMPCMBUF * nBytesPerBuf)) 
        {
            bufpos -= (PCM_NUMPCMBUF * nBytesPerBuf);
        }
    }
    ptr = (unsigned char *) (pCard->xferbuf[0] + bufpos);
//    printf("Submitbuffer bufpos = %d, offset = %d\n",bufpos,(bufpos * pCard->pcmBufferSize *PCM_FRAMESIZE));
//    printf("Submitbuffer xferbuf_pos = %d\n", pCard->xferbuf_pos[0]);
...
//        printf("SubmitBuffer writing %d bytres to card buffer\n",pCard->pcmBufferSize * (PCM_FRAMESIZE));
//        memcpy(ptr, frame_data, pCard->pcmBufferSize * (PCM_FRAMESIZE));
//        memcpy(ptr, frame_data, pCard->pcmBufferSize * (PCM_FRAMESIZE));
        memcpy(ptr, frame_data, nSize);
        pCard->buffer_pending[0] += nSize;
        nRet = TRUE;
    }
    else
    {
...
int pcmSendAudioThread(ACard *pCard) 
{
    ...
    nWriteSz = pCard->pcmBufferSize * PCM_FRAMESIZE;
    ...
    while (1)
    {
        ...
        else 
        {
                ...
                pCard->active_channel[0]) 
            {
                ...
                if (pCard->buffer_pending[0] >= (nInitialBuf * nWriteSz))
                {
                    nInitialBuf = 1;
                    ...
                    if (pCard->buffer_pending[0] > 0) 
                    {
                        pCard->buffer_pending[0] -= nWriteSz;
                    }       
//                    printf("Send Audio xferbuf pos = %d\n",pCard->xferbuf_pos[0]);
//                    printf("Send Audio xferindex = %d\n",xferindex[0]);
                    ...
                    if ( !(nCount % 1) )
                    {
//                        printf("Send Audio writing %d bytes to card\n", nNumbytes);
                    }
                } 
            }        
        } 
