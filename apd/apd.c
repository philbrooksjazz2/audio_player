...
apdPlaySong(ACard *pCard, SongInfo *pInfo)
{
    ...
    // Uncomment below line to use gdb, and comment out clone or pthread call.
...
//    g_pCard->pcmbuf = (unsigned char *)malloc(num_bytes);

    fseek(g_pSongInfo->fd,nOffset,SEEK_SET);

//    ret = fread(g_pCard->pcmbuf, offset, nWanted, g_pSongInfo->fd);
//    ret = fread(g_pCard->pcmbuf, 1, nWanted, g_pSongInfo->fd);
    ret = fread(pTmp, 1, num_bytes, g_pSongInfo->fd);

//    g_pCard->pcmBufferSize = ret;

//    printf("CBGetData read %d bytes\n",ret);
//    *ppData = g_pCard->pcmbuf;
    *ppData = pTmp;
    return ret;
}
...
tWMA_U32 WMAFileCBGetLicenseData (
    ...
    unsigned char **ppData) 
{
    printf("apd GetLicenseData called\n");
    return 0;
}
//#endif // DO_WMA
...
//    }
    ...
    do 
    {
        ...
        apdLogPrint("XXX43", APD_LOG_INFO, "d");
#ifdef DO_WMA

        printf("A\n");

        /* delete any WMA buffer we might have */
        if (p->songinfo->wma.wmabuf != NULL) 
        {
            free((void *)p->songinfo->wma.wmabuf);
            p->songinfo->wma.wmabuf = NULL;
        }
        p->songinfo->wma.wmabuf_bytes = 0;

        printf("AB\n");
        /*  open AMP to decode or initialise WMA decoding engine */
        if (TRUE/* replace with correct IsWMA function*/) 
        {
            printf("AC\n");
            ret = apdWmadecode(p->cardinfo, p->songinfo);
        } 
        else 
        {
            ret = apdMp3decode();
...
#ifdef DO_WMA
int playpfc_pcmdata(SongInfo *pSongInfo, ACard *pCard,unsigned char *src, int numbytes) 
{

    int bytesleft = numbytes;
    int offset = 0;
    int pcmbufsize = MADPCM_SAMPLES * 8;
    int copybytes = 0;
    void *destptr = NULL;
    void *srcptr = NULL;
    char logstr[256] = "";
    ...
    while (bytesleft > 0) 
    {
        ...
        copybytes = pcmbufsize - pSongInfo->wma.wmabuf_bytes;
        ...
        destptr = (void *) ((unsigned int) pSongInfo->wma.wmabuf + pSongInfo->wma.wmabuf_bytes);
        srcptr = (void *) ((unsigned int) src + offset);
        memcpy(destptr, srcptr, copybytes);
        /* adjust numbers */
        offset += copybytes;
        bytesleft -= copybytes;
        pSongInfo->wma.wmabuf_bytes += copybytes;
        
        /* do we have a full wmabuf? */
        if (pSongInfo->wma.wmabuf_bytes >= pcmbufsize) 
        {
            pSongInfo->wma.wmabuf_bytes = 0;
            /* submit data to pcm handler */
            if (!pcmSubmitBuffer(pCard, (void *) pSongInfo->wma.wmabuf, copybytes)) 
  //          if (!pcmSubmitBuffer(pCard, (void *) destptr, copybytes)) 
            {
                sprintf(logstr, "Error writing PCM buffer\n");
                apdLogPrint("PF59", APD_LOG_ERROR, logstr);
                return FALSE;
            }
            printf("submit buf %d samples\n",copybytes);
        }
    }
    return TRUE;
}
/* fill rest of buffer with zeros and submit to PCM handler */
void playpfc_flushpcm(SongInfo *pSongInfo, ACard *pCard) 
{
    int bytesneeded;
//    int pcmbufsize = pcmnumblocks * 8;
    ...
    if (pSongInfo->wma.wmabuf_bytes == 0) 
    {
        return;
    }

    /* how many zeros do we need? */
    bytesneeded = pcmbufsize - pSongInfo->wma.wmabuf_bytes;
    
    /* copy the zeros in there */
    dstptr = (void *) ((unsigned int) pSongInfo->wma.wmabuf + pSongInfo->wma.wmabuf_bytes);
    ...
    pcmSubmitBuffer(pCard, (void *)pSongInfo->wma.wmabuf,bytesneeded);
