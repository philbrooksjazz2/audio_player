...
/*
 * $Log: alsa_apdpcm.c,v $
 * Revision 1.1  2007/09/05 23:39:12  pbrooks
 * Added alsa versions.
 *
...
int pcmInitAlsa(ACard *pCard, int buffer_size) 
{
    long loops;
    int rc;
    int size;
...
//    pCard->dfx_handle[1] = NULL;  
    ...
    rc = snd_pcm_open(&pCard->ahandle, "default",
                    SND_PCM_STREAM_PLAYBACK, 0);
    if (rc < 0) 
    {
        apdLogPrint("APD50", APD_LOG_ERROR, "Alsa Open Failed");
//            snd_strerror(tmp_handle));
        return FALSE;
    }
    ...
    snd_pcm_hw_params_any(pCard->ahandle, params);

    /* Set the desired hardware parameters. */
    /* Interleaved mode */
    snd_pcm_hw_params_set_access(pCard->ahandle, params,
                      SND_PCM_ACCESS_RW_INTERLEAVED);

    /* Signed 16-bit little-endian format */
    snd_pcm_hw_params_set_format(pCard->ahandle, params,
                              SND_PCM_FORMAT_S16_LE);
    /* Two channels (stereo) */
    snd_pcm_hw_params_set_channels(pCard->ahandle, params, 2);

    /* 44100 bits/second sampling rate (CD quality) */
    val = PCM_SAMPLERATE;
    snd_pcm_hw_params_set_rate_near(pCard->ahandle, params,
                                  &val, &dir);
    snd_pcm_hw_params_set_periods(pCard->ahandle,
                              params, 2, 0);
    /* Set period size */
//    frames = nBufSize;
//    frames = buffer_size/5;
    frames = buffer_size;
    snd_pcm_hw_params_set_buffer_size(pCard->ahandle,
                              params, buffer_size);
    snd_pcm_hw_params_set_period_size_near(pCard->ahandle,
                              params, &frames, &dir);

    /* Write the parameters to the driver */
    rc = snd_pcm_hw_params(pCard->ahandle, params);
    if (rc < 0) 
    {
        apdLogPrint("APD51", APD_LOG_ERROR, "Alsa Init Failed");
        if (pCard->ahandle)
        {
            snd_pcm_close(pCard->ahandle);
        }
        return FALSE;
    }
...
//    gPcmPaused = TRUE;
    ...
    while (1)
    {
        ...
        {
#ifdef ALSA
            snd_pcm_writei(pCard->ahandle, zerobuf, ZEROBUF_SZ);
#else
...
#ifdef ALSA
            nNumbytes = snd_pcm_writei(pCard->ahandle, pCard->pcmbuf, 
//                       pCard->pcmBufferSize * PCM_FRAMESIZE); 
                       (pCard->pcmBufferSize / 2) ); 
#else
